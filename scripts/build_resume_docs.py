...
class ResumeBuilder:
    ...
    def _run(self, paragraph, text, weight="regular", italic=False, size=9.0, color=INK, condensed=False):
    ...
    def _bullet(self, text):
        ...
        self._run(p, (marker + text + suffix) if self.is_fa else (marker + text), weight="light", size=8.6 if not self.is_fa else 7.9, color=RGBColor(0x33, 0x33, 0x33))

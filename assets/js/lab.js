...
(function () {
  ...
  var STEPS = [
    ...
    },
    {
      ...
      output: [
        ...
        "[download] 100% of 24.31MiB in 00:03"
      ]
    },
    {
      cmd: "spotdl download <playlist-url>",
      output: ["Processing query: Playlist", "Found 24 songs in playlist", 'Downloaded "Track 01": 100%']
    },
    {
  ...
  /* types `text` into `span` one character at a time */
  ...
  function runStep() {
    ...
    function printOutput(i, done) {
      ...
      appendOutputLine(step.output[i]);
      schedule(function () { printOutput(i + 1, done); }, jitter(140, 220));
    }

    if (step.typo) {
      // simulate a realistic typo: type the wrong word, pause, backspace
      // it out, then type the correct rest of the command

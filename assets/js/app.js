(function () {
  ...
  if (pollCard) {
    var dismissed = false;
    try { dismissed = localStorage.getItem("spotifyPollDismissed") === "1"; } catch (e) {}
    if (dismissed) {
      pollCard.classList.add("is-dismissed");
    } else {
      var pollText = pollCard.querySelector(".poll-card__text");
      var isFa = document.body.classList.contains("lang-fa");
      pollCard.querySelectorAll(".poll-card__btn").forEach(function (btn) {
        btn.addEventListener("click", function () {
          if (btn.dataset.poll === "no" && pollText) {
            pollText.textContent = isFa
              ? "باشه، ثبت شد! ولی فعلاً نگهش می‌دارم :)"
              : "Noted! Keeping it anyway though :)";
            btn.parentElement.style.display = "none";
            setTimeout(function () {
              pollCard.classList.add("is-dismissed");
              try { localStorage.setItem("spotifyPollDismissed", "1"); } catch (e) {}
            }, 1600);
          } else {
            pollCard.classList.add("is-dismissed");
            try { localStorage.setItem("spotifyPollDismissed", "1"); } catch (e) {}

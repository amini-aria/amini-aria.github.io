(function () {
  ...
  /* ---------- Keep the résumé download links permanent; fetch fresh on click ----------
     The four files (Aria-CV-En / Aria-CV-Fa, .pdf and .docx) are rebuilt
     by CI in place, so their URLs never change and can be shared as-is.
     But GitHub Pages serves them with a 4-hour max-age, so a browser that
     downloaded one before can silently keep serving the stale copy for
     hours. This used to be handled by rewriting every href to "?v=<now>"
     at page load, which meant "Copy link address" on a button handed out
     a different URL every time. Now the href stays the clean, permanent
     URL and the unique query string is attached only for the instant of
     the click: the browser reads the href after the handlers have run, so
     the download itself still bypasses the cache, and the attribute is put
     back right after. */
  document.querySelectorAll(".resume-downloads a[download]").forEach(function (a) {
    a.addEventListener("click", function () {
      var clean = a.getAttribute("href").split("?")[0];
      a.setAttribute("href", clean + "?v=" + Date.now());
      setTimeout(function () { a.setAttribute("href", clean); }, 0);
    });
  });
